...
namespace Unir.TFG.MeterMAX.Protocols.ANSI.C12_21.Services
{
    internal class DESEncryptor
    {
        ...
        internal static byte[] EncryptTicket(byte[] ticket, byte[] key)
        {
            byte[] output = new byte[8];
            byte[,] schedule = new byte[16, 6];

            KeySchedule(key, schedule, Mode.Encrypt);
            Encrypt(ticket, output, ToJaggedArray(schedule));
            return output;
        }

        private static void KeySchedule(byte[] key, byte[,] schedule, Mode mode)
        {
            uint i, j, toGen, C, D;
            uint[] key_rnd_shift = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
            uint[] key_perm_c = { 56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35 };
            uint[] key_perm_d = { 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36, 28, 20, 12, 4, 27, 19, 11, 3 };
            uint[] key_compression = { 13,16,10,23,0,4,2,27,14,5,20,9, 22,18,11,3,25,7,15,6,26,19,12,1,40,51,30,36,46,54,29,39,50,44,32,47,
            ...
            for (i = 0; i < 16; ++i)
            {
                ...
                toGen = (mode == Mode.Encrypt) ? i : (15 - i);
